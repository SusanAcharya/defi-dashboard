...
export const Explore: React.FC = () => {
  ...
  const getCategoryBowls = (category: string) => {
    return (
      hotBowls?.filter((bowl) => bowl.category === category).slice(0, 3) || []
    );
  };
  ...
  const filteredPools = useMemo(() => {
    ...
    return pools.filter((pool) => {
      ...
      const matchesProtocol =
        selectedProtocol === "Protocols" || pool.protocol === selectedProtocol;
      const matchesPoolType =
        selectedPoolType === "All Pool Types" ||
        pool.poolType === selectedPoolType;
      const matchesAssetType =
        selectedAssetType === "All Asset Types" ||
        (selectedAssetType === "Stablecoin" &&
          (pool.pair.includes("USDC") || pool.pair.includes("USDT"))) ||
        (selectedAssetType === "Blue-chip" &&
          (pool.pair.includes("ETH") || pool.pair.includes("STRK")));
      return (
        matchesSearch && matchesProtocol && matchesPoolType && matchesAssetType
      );
    });
  }, [
    pools,
                ...
                <div key={category.id} className={styles.hotBowls__category}>
                  <div className={styles.hotBowls__categoryHeader}>
                    <div className={styles.hotBowls__categoryTitle}>
                      <span className={styles.hotBowls__categoryIcon}>
                        {category.icon}
                      </span>
                      <span>{category.label}</span>
                    </div>
                    <button className={styles.hotBowls__viewAll}>
                      View all &gt;
                    </button>
                  </div>
                  <div className={styles.hotBowls__list}>
                    {bowls.map((bowl) => (
                      <div key={bowl.id} className={styles.hotBowls__item}>
                        <span className={styles.hotBowls__pair}>
                          {bowl.pair}
                        </span>
                          ...
                          <div className={styles.explore__tablePool}>
                            ...
                            <div className={styles.explore__tablePoolInfo}>
                              <div className={styles.explore__tablePoolName}>
                                {pool.pair}
                                {pool.verified && (
                                  <span className={styles.explore__verified}>
                                    ✓
                                  </span>
                                )}
                                {pool.hasRewards && (
                                  <span className={styles.explore__gift}>
                                    🎁
                                  </span>
                                )}
                              </div>
                ...
                <table className={styles.explore__table}>
                  ...
                  <tbody>
                    {filteredLending.map((option) => (
                      ...
                      >
                        <td className={styles.explore__tableToken}>
                          {option.token}
                        </td>
                      ...
                      >
                        <td>
                          <div className={styles.explore__tableStrategy}>
                            <span className={styles.explore__strategyIcon}>
                              💎
                            </span>
                            <span>
                              {strategy.name}
