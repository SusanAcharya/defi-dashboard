...
import { formatCurrency, formatPercentage, formatAddress, formatTimestamp } from '@/utils/format';
...
const transactionTypes = ['All', 'Buy', 'Sell', 'Add Liquidity', 'Remove Liquidity'];
...
export const PoolDetail: React.FC = () => {
  ...
  const { data: transactions } = useQuery({
    ...
    queryFn: () => api.getPoolTransactions(poolId || '', selectedTransactionType === 'All' ? undefined : selectedTransactionType.toLowerCase().replace(' ', '-') as any),
    enabled: !!poolId,
  });

  const { data: chartData } = useQuery({
    queryKey: ['pool-chart', poolId, selectedMetric.toLowerCase() as any, selectedTimeframe.toLowerCase()],
    queryFn: () => api.getPoolChartData(poolId || '', selectedMetric.toLowerCase() as any, selectedTimeframe.toLowerCase() as any),
    enabled: !!poolId,
  });
  ...
  return (
    <div className={styles.poolDetail}>
      ...
      <div className={styles.poolDetail__header}>
        <button className={styles.poolDetail__back} onClick={() => navigate('/explore')}>
          ← Back
        </button>
      ...
      <div className={styles.poolDetail__content}>
        ...
        <div className={styles.poolDetail__left}>
                ...
                {metricTabs.map((metric) => (
                  <button
                    ...
                    className={`${styles.poolDetail__metricTab} ${
                      selectedMetric === metric ? styles.poolDetail__metricTab_active : ''
                    }`}
                ...
                {timeframes.map((tf) => (
                  <button
                    ...
                    className={`${styles.poolDetail__timeframe} ${
                      selectedTimeframe === tf ? styles.poolDetail__timeframe_active : ''
                    }`}
          ...
          <Card title="Transactions" className={styles.poolDetail__transactionsCard}>
            <div className={styles.poolDetail__transactionFilters}>
              {transactionTypes.map((type) => (
                <button
                  key={type}
                  className={`${styles.poolDetail__transactionFilter} ${
                    selectedTransactionType === type ? styles.poolDetail__transactionFilter_active : ''
                  }`}
                ...
                <tbody>
                  {transactions?.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td>
                        <span className={`${styles.poolDetail__txType} ${styles[`poolDetail__txType_${tx.type}`]}`}>
                          {tx.type.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                        </span>
                      </td>
                      <td>{formatCurrency(tx.usdValue, 'USD', showFinancialNumbers)}</td>
                      ...
                      <td>
                        <button className={styles.poolDetail__action}>View</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className={styles.poolDetail__sidebarTabs}>
              <button
                className={`${styles.poolDetail__sidebarTab} ${
                  activeTab === 'details' ? styles.poolDetail__sidebarTab_active : ''
                }`}
              ...
              <button
                className={`${styles.poolDetail__sidebarTab} ${
                  activeTab === 'position' ? styles.poolDetail__sidebarTab_active : ''
                }`}
              ...
              <div className={styles.poolDetail__details}>
                <div className={styles.poolDetail__stat}>
                  ...
                  <div className={styles.poolDetail__statValue}>
                    {formatCurrency(poolDetail.tvl, 'USD', showFinancialNumbers)}
                  </div>
                  <div className={`${styles.poolDetail__statChange} ${
                    poolDetail.tvlChange24h >= 0 ? styles.poolDetail__statChange_positive : styles.poolDetail__statChange_negative
                  }`}>
                    {formatPercentage(poolDetail.tvlChange24h, 2, showFinancialNumbers)}
                  </div>
                </div>
                ...
                <div className={styles.poolDetail__stat}>
                  ...
                  <div className={styles.poolDetail__statValue}>
                    {formatCurrency(poolDetail.volume24h, 'USD', showFinancialNumbers)}
                  </div>
                  <div className={`${styles.poolDetail__statChange} ${
                    poolDetail.volumeChange24h >= 0 ? styles.poolDetail__statChange_positive : styles.poolDetail__statChange_negative
                  }`}>
                    {formatPercentage(poolDetail.volumeChange24h, 2, showFinancialNumbers)}
                  </div>
                </div>
                ...
                <div className={styles.poolDetail__stat}>
                  ...
                  <div className={styles.poolDetail__statValue}>
                    {formatCurrency(poolDetail.fee24h, 'USD', showFinancialNumbers)}
                  </div>
                  <div className={`${styles.poolDetail__statChange} ${
                    poolDetail.feeChange24h >= 0 ? styles.poolDetail__statChange_positive : styles.poolDetail__statChange_negative
                  }`}>
                    {formatPercentage(poolDetail.feeChange24h, 2, showFinancialNumbers)}
                  </div>
                </div>
                ...
                <div className={styles.poolDetail__stat}>
                  ...
                  <div className={styles.poolDetail__statValue}>
                    {formatPercentage(poolDetail.apr24h, 2, showFinancialNumbers)}
                  </div>
                  <div className={`${styles.poolDetail__statChange} ${
                    poolDetail.aprChange24h >= 0 ? styles.poolDetail__statChange_positive : styles.poolDetail__statChange_negative
                  }`}>
                    {poolDetail.aprChange24h >= 0 ? '↑' : '↓'} {formatPercentage(Math.abs(poolDetail.aprChange24h), 2, showFinancialNumbers)}
                  </div>
                </div>

                <div className={styles.poolDetail__info}>
                  <div className={styles.poolDetail__infoLabel}>Pool Address</div>
                ...
                <div className={styles.poolDetail__info}>
                  ...
                  <div className={styles.poolDetail__infoValue}>{poolDetail.poolType}</div>
                </div>

                <div className={styles.poolDetail__info}>
                  <div className={styles.poolDetail__infoLabel}>Current Price</div>
                  <div className={styles.poolDetail__infoValue}>{poolDetail.currentPrice}</div>
                </div>

                <div className={styles.poolDetail__info}>
                  <div className={styles.poolDetail__infoLabel}>Platform</div>
                  <div className={styles.poolDetail__infoValue}>{poolDetail.protocol}</div>
                </div>
                ...
                <div className={styles.poolDetail__lpBreakdown}>
                    ...
                    {poolDetail.lpBreakdown.map((asset, index) => (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: index === 0 ? '#ffa500' : '#ff8c00',
                        }}
                      />
                    ))}
                  </div>
                  <div className={styles.poolDetail__lpTable}>
                    {poolDetail.lpBreakdown.map((asset) => (
                      <div key={asset.asset} className={styles.poolDetail__lpRow}>
                        <div className={styles.poolDetail__lpAsset}>{asset.asset}</div>
                        <div className={styles.poolDetail__lpAmount}>
                          {formatCurrency(asset.coinAmount / 1000000, 'USD', showFinancialNumbers)}M
                        </div>
                        <div className={styles.poolDetail__lpValue}>
                          {formatCurrency(asset.value / 1000000, 'USD', showFinancialNumbers)}M
                        </div>
                      </div>
                    ))}
                  </div>
